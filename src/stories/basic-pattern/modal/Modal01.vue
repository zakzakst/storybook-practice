...
#{ $BLOCK_NAME } {
  ...
  &__content {
    ...
    border-radius: 24px;
    // タイプPOP
    @at-root #{ $BLOCK_NAME }.--type-pop & {
      transform: translateY(50px) scale(0.9);
      opacity: 0;
      transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);
    }
    @at-root #{ $BLOCK_NAME }.--type-pop.is-active & {
      transform: translateY(0) scale(1);
      opacity: 1;
    }
    // タイプFADE
    @at-root #{ $BLOCK_NAME }.--type-fade & {
      transform: translateY(20px);
      opacity: 0;
      transition: all 0.7s ease;
    }
    @at-root #{ $BLOCK_NAME }.--type-fade.is-active & {
      transform: translateY(0);
      opacity: 1;
    }
    // タイプSLIDE
    @at-root #{ $BLOCK_NAME }.--type-slide & {
      transform: translateY(80px);
      opacity: 0;
      transition: all 0.3s ease;
    }
    @at-root #{ $BLOCK_NAME }.--type-slide.is-active & {
      transform: translateY(0);
      opacity: 1;
    }
  }
